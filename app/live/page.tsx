...
const INITIAL_CHAT_MESSAGES = [
  { id: 1, user: "Rafsan", message: "Wow, what a goal by Banani Falcons! ⚽", time: "18:40", isPremium: true },
  { id: 2, user: "Tasin", message: "Dhanmondi's goalkeeper made an absolute world-class save just now!", time: "18:41", isPremium: false },
  { id: 3, user: "Sadman", message: "Go backend in Golang is keeping the stream latency next to zero! Super smooth.", time: "18:42", isPremium: true },
  { id: 4, user: "Mahmudul", message: "Falcons defense is looking shaky tonight, Tigers can equalize anytime.", time: "18:43", isPremium: false }
];
...
export default function LiveStreamPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (mainStreamRef.current) {
        tl.to(mainStreamRef.current, { opacity: 1, y: 0, duration: 0.8, ease: 'power3.out' }, '-=0.3');
      }
      ...
      <div className="max-w-6xl mx-auto space-y-16 relative z-10">
        ...
        <header className="text-center space-y-5 max-w-2xl mx-auto flex flex-col items-center">
          ...
          <h1 className="text-3xl sm:text-5xl font-black tracking-tight leading-[1.12] text-white will-change-transform">
            ...
            <div className="mt-1">
              ...
              >
                {render3DLetters("Arena Broadcasting", "text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-emerald-300 to-teal-400")}
              </span>
            </div>
          </h1>
          ...
          >
            টার্ফবুক অ্যারেনা থেকে সরাসরি সম্প্রচারিত হওয়া ফুটবল ও ক্রিকেট ম্যাচগুলো কোনো বাফারিং ছাড়াই উপভোগ করুন। আপনার বন্ধুদের সাথে লিংক শেয়ার করুন এবং লাইভ চ্যাট লবিতে আড্ডা দিন!
        ...
        >
                ...
                {selectedMatch.status === 'LIVE' && (
                  <div className="flex items-center bg-[#050811]/90 backdrop-blur-md border border-slate-800 rounded-full px-4.5 py-1.5 shadow-[0_5px_15px_rgba(0,0,0,0.5)] font-mono">
                    ...
                    <div className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                      <Clock className="h-3 w-3 text-emerald-400 animate-spin" style={{ animationDuration: '8s' }} />
                      <span>
                ...
                <div className="relative z-20 flex flex-col items-center justify-center text-center space-y-3.5 bg-[#050811]/70 backdrop-blur-md p-6 rounded-2xl max-w-sm mx-auto shadow-2xl border border-slate-850">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-semibold mt-1">
                      Broadcast commences in Mirpur cluster tonight. Registered team squads have locked in.
                    </p>
                  </div>
            ...
            <div className="p-5.5 rounded-3xl bg-[#0d1425]/15 border border-slate-900 flex flex-col justify-between space-y-4">
              ...
              <div className="flex justify-between items-center pb-3.5 border-b border-slate-850/65">
                <div className="space-y-1">
                  <span className="text-[8px] font-black uppercase tracking-widest text-slate-500">Dhaka Sports Broadcasting Service</span>
                  <h3 className="text-sm font-black text-white">{selectedMatch.title}</h3>
                  <p className="text-[10px] text-slate-400 font-semibold">📍 Arena: {selectedMatch.arena} | Sport: {selectedMatch.sport}</p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px] sm:text-xs text-slate-400 font-semibold leading-relaxed pt-1">
                <div className="space-y-2">
                  <p>
                    🔥 <strong>Golang Concurrency Engine:</strong> এই লাইভ ব্রডকাস্টিং সার্ভারটি গোলং এবং WebRTC প্রযুক্তি দ্বারা চালিত, যা লো-ল্যাটেন্সি রিয়েল-টাইম কন্টেন্ট সম্প্রচার নিশ্চিত করে।
                  </p>
                  <p>
                    🏆 <strong>Dhaka Super League:</strong> টুর্নামেন্টে চ্যাম্পিয়ন দলের জন্য থাকছে আকর্ষণীয় প্রাইজমানি এবং টার্ফবুক প্লেয়ার অফ দ্য ম্যাচ অনার।
                  </p>
                </div>

                {/* Simulated match events block */}
                <div className="bg-[#050811] p-4.5 rounded-2xl border border-slate-850 space-y-2.5">
                  <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest block">Match Events Ticker</span>
          ...
          <div className="lg:col-span-4 flex flex-col justify-between bg-[#0d1425]/15 border border-slate-900 rounded-3xl p-5.5 space-y-4 relative overflow-hidden h-[480px]">
              ...
              {chatMessages.map((msg) => (
                ...
                >
                  <div className="flex justify-between items-center text-[9px] font-bold text-slate-500 uppercase tracking-widest">
                    <span className={`font-black ${msg.isPremium ? 'text-emerald-400' : 'text-slate-350'}`}>
                      {msg.user} {msg.isPremium && '⭐'}
                    </span>
                    <span>{msg.time}</span>
                  </div>
                  <p className={`text-[10px] leading-relaxed font-semibold ${msg.user === 'TurfBook Broadcast' ? 'text-white font-bold' : 'text-slate-400'}`}>
                    {msg.message}
                  </p>
                </div>
              ))}
            ...
            <form onSubmit={handleSendMessage} className="flex gap-2 bg-[#050811] p-1.5 rounded-xl border border-slate-850">
              <input 
                type="text" 
                value={newMessage}
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-1 flex-1 overflow-hidden">
                    ...
                    <h4 className="text-[11px] font-black text-white truncate">{broadcast.title}</h4>
